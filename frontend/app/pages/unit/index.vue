    ...
    <div class="pl-64 lg:pl-72 flex flex-col min-h-screen">
      ...
      <main class="flex-1 w-full px-8 py-6 space-y-6">
        ...
        <div v-if="successMessage" class="bg-emerald-50 border border-emerald-200 text-emerald-800 p-4 rounded-2xl text-xs flex items-center justify-between shadow-2xs">
          ...
          <button @click="successMessage = null" class="font-bold text-emerald-800 hover:text-emerald-950">✕</button>
        </div>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white border border-slate-200/90 rounded-2xl p-6 shadow-2xs">
          <div>
            <div class="flex items-center gap-2.5">
              <h1 class="text-xl font-bold text-slate-900 tracking-tight">Departmental & Ward Leadership Overview</h1>
            ...
            <p class="text-xs text-slate-500 mt-0.5">
              Supervisory telemetry, staff shift coverage, CAAC scanner alerts, and clinical acuity oversight.
            </p>
          </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5">
          ...
          <div class="bg-white border border-slate-200/90 rounded-2xl p-5 shadow-2xs hover:shadow-xs transition-shadow">
            ...
            <p class="text-2xl font-bold text-slate-900 font-mono">{{ metrics.totalWardInpatients }}</p>
            <p class="text-[11px] text-slate-400 mt-1">{{ metrics.bedOccupancyRate }}% Bed Occupancy</p>
          </div>

          <!-- Card 2: Acuity Breakdown -->
          <div class="bg-white border border-slate-200/90 rounded-2xl p-5 shadow-2xs hover:shadow-xs transition-shadow">
            <div class="flex items-center justify-between text-slate-400 mb-2">
              <span class="text-xs font-bold uppercase tracking-wider text-red-700">Acuity Status</span>
            ...
            <p class="text-[11px] text-red-600/80 mt-1">
              {{ metrics.monitoringCount }} monitoring · {{ metrics.stableCount }} stable
            </p>
          </div>

          <!-- Card 3: Active Duty Staff -->
          <div class="bg-white border border-slate-200/90 rounded-2xl p-5 shadow-2xs hover:shadow-xs transition-shadow">
            <div class="flex items-center justify-between text-slate-400 mb-2">
              <span class="text-xs font-bold uppercase tracking-wider text-emerald-700">Clinicians On-Duty</span>
            ...
            <p class="text-2xl font-bold text-emerald-700 font-mono">{{ metrics.onDutyStaffCount }}</p>
            <p class="text-[11px] text-emerald-600/80 mt-1">of {{ metrics.totalAssignedStaff }} total assigned</p>
          </div>

          <!-- Card 4: Today's Clinic Queue -->
          <div class="bg-white border border-slate-200/90 rounded-2xl p-5 shadow-2xs hover:shadow-xs transition-shadow">
            <div class="flex items-center justify-between text-slate-400 mb-2">
              <span class="text-xs font-bold uppercase tracking-wider text-purple-700">Clinic Queue</span>
            ...
            <p class="text-2xl font-bold text-purple-700 font-mono">{{ metrics.todayAppointmentsCount }}</p>
            <p class="text-[11px] text-purple-600/80 mt-1">Consultation slots today</p>
          </div>

          <!-- Card 5: Active Security Alerts -->
          <div class="bg-white border border-slate-200/90 rounded-2xl p-5 shadow-2xs hover:shadow-xs transition-shadow">
            <div class="flex items-center justify-between text-slate-400 mb-2">
              <span class="text-xs font-bold uppercase tracking-wider text-amber-700">Security Alerts</span>
            ...
            <p class="text-2xl font-bold text-amber-700 font-mono">{{ metrics.activeSecurityAlertsCount }}</p>
            <p class="text-[11px] text-amber-600/80 mt-1">Requiring supervisor review</p>
          </div>
        </div>
