    ...
    <div class="pl-56 flex flex-col min-h-screen">
      ...
      <main class="flex-1 w-full px-8 py-6 space-y-5">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div class="flex items-center gap-2">
              <NuxtLink to="/nurse" class="text-xs font-semibold text-purple-700 hover:text-purple-900 flex items-center gap-1">
              ...
              <span class="text-xs font-medium text-slate-500">My Profile</span>
            </div>
            <h1 class="font-brand text-xl font-semibold text-slate-900 tracking-tight mt-1">Nurse Profile &amp; Clinical Credentials</h1>
            <p class="text-xs text-slate-500 mt-0.5">
              Review active nursing authorization parameters, shift windows, and manage personal credentials.
            </p>
          </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <!-- Left Column: Identity & Shift Card -->
          <div class="bg-white border border-slate-200/90 rounded-xl p-6 shadow-2xs space-y-6">
            <div class="flex items-center gap-4 pb-5 border-b border-slate-100">
              <div class="w-14 h-14 rounded-2xl bg-purple-700 text-white flex items-center justify-center font-bold text-xl font-mono shadow-md shadow-purple-900/20">
                {{ initials }}
              </div>
              <div class="space-y-0.5 min-w-0">
                <h3 class="text-base font-bold text-slate-900 truncate">{{ user?.fullName || 'Nurse' }}</h3>
                <p class="text-xs font-mono text-slate-500 truncate">@{{ user?.username }}</p>
                <div class="pt-1">
                  <RoleBadge :role="user?.role || 'NURSE'" />
                </div>
              </div>
            </div>
            ...
            <div class="space-y-3.5 text-xs">
              <div class="flex items-center justify-between py-1.5 border-b border-slate-100">
                <span class="text-slate-500">Home Ward:</span>
              ...
              <div class="flex items-center justify-between py-1.5 border-b border-slate-100">
                ...
                <span class="font-bold text-purple-700 bg-purple-50 px-2 py-0.5 rounded font-mono border border-purple-200">
                  {{ activeWardName }} ({{ activeWardCode }})
                </span>
              </div>
              ...
              <div class="flex items-center justify-between py-1.5 border-b border-slate-100">
                ...
                <span class="font-mono text-slate-700 font-bold">12-Hour Nursing Shift</span>
              </div>
              ...
              <div class="flex items-center justify-between py-1.5">
                <span class="text-slate-500">Clinical Scope:</span>
                <span class="text-slate-700 font-medium">Vitals, Meds, Allergies, Bed</span>
              </div>
            </div>
          </div>
          ...
          <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white border border-slate-200/90 rounded-xl p-6 shadow-2xs space-y-5">
              <div class="flex items-center justify-between pb-3 border-b border-slate-100">
                <div>
                  <h3 class="text-xs font-bold text-slate-700 uppercase tracking-wider">Nursing Staff Information</h3>
                  <p class="text-[11px] text-slate-500">Update your clinical display name on medical charts and vital sign records.</p>
                </div>
              </div>
              ...
              <form @submit.prevent="handleUpdateProfile" class="space-y-4 text-xs">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      type="text"
                      required
                      placeholder="e.g. Nurse Amara Diop, RN"
                      class="w-full bg-slate-50 border border-slate-300 rounded-xl px-3.5 py-2.5 text-slate-900 focus:outline-none focus:border-purple-500 font-medium"
                    />
                  </div>
                </div>
                ...
                <div class="flex justify-end pt-2">
                  <button
                    ...
                    class="px-5 py-2.5 bg-purple-700 hover:bg-purple-800 active:bg-purple-900 text-white rounded-xl font-semibold shadow-xs flex items-center gap-2 cursor-pointer transition-colors"
                  >
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      class="w-full bg-slate-50 border border-slate-300 rounded-xl px-3.5 py-2.5 text-slate-900 focus:outline-none focus:border-purple-500"
                    />
                  </div>
...
const initials = computed(() => {
  const name = user.value?.fullName || user.value?.username || 'NU'
