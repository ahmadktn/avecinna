  ...
  <div class="min-h-screen bg-slate-50 font-sans">
    ...
    <div class="pl-64 lg:pl-72 flex flex-col min-h-screen">
      ...
      <main class="flex-1 w-full px-8 py-6 space-y-6">
        ...
        <!-- Success Toast -->
        <div v-if="successMsg" class="bg-emerald-50 border border-emerald-200 text-emerald-800 p-4 rounded-2xl text-xs flex items-center justify-between shadow-2xs">
          <div class="flex items-center gap-2.5">
        ...
        <!-- Station Header -->
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white border border-slate-200/90 rounded-2xl p-6 shadow-2xs">
          <div>
            <div class="flex items-center gap-2.5">
              <h1 class="text-xl font-bold text-slate-900 tracking-tight">Nursing Station & Ward Overview</h1>
            ...
            <p class="text-xs text-slate-500 mt-0.5">
              Real-time physiological telemetry, patient census, and multidisciplinary care team assignments for {{ activeWardName }}.
            </p>
          </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          ...
          <div class="bg-white border border-slate-200/90 rounded-2xl p-5 shadow-2xs hover:shadow-xs transition-shadow">
            ...
            <p class="text-2xl font-bold text-slate-900 font-mono">{{ metrics.wardInpatientsCount }}</p>
            <p class="text-[11px] text-slate-400 mt-1">Bed Occupancy: <span class="font-mono font-bold text-slate-700">{{ metrics.bedOccupancyRate }}%</span></p>
          </div>

          <!-- Card 2: My Care Team Assignments -->
          <div class="bg-white border border-purple-200/80 rounded-2xl p-5 shadow-2xs hover:shadow-xs transition-shadow bg-purple-50/20">
            <div class="flex items-center justify-between text-purple-700 mb-2">
              <span class="text-xs font-bold uppercase tracking-wider text-purple-700">My Care Team Consults</span>
              <div class="w-8 h-8 rounded-xl bg-purple-100 flex items-center justify-center text-purple-700">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                </svg>
              </div>
            </div>
            <p class="text-2xl font-bold text-purple-900 font-mono">{{ metrics.myCareTeamCount }}</p>
            <p class="text-[11px] text-purple-700/80 mt-1">Direct nurse assignments</p>
          </div>

          <!-- Card 3: Observation / Monitoring -->
          <div class="bg-white border border-slate-200/90 rounded-2xl p-5 shadow-2xs hover:shadow-xs transition-shadow">
            <div class="flex items-center justify-between text-slate-400 mb-2">
              <span class="text-xs font-bold uppercase tracking-wider text-amber-700">Monitoring Watch</span>
            ...
            <p class="text-2xl font-bold text-amber-600 font-mono">{{ metrics.monitoringCount }}</p>
            <p class="text-[11px] text-amber-700/80 mt-1">Observation telemetry protocol</p>
          </div>

          <!-- Card 4: Critical Acuity -->
          <div class="bg-white border border-slate-200/90 rounded-2xl p-5 shadow-2xs hover:shadow-xs transition-shadow">
            <div class="flex items-center justify-between text-slate-400 mb-2">
            ...
            <p class="text-[11px] text-red-700/80 mt-1">High vigilance alert</p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="lg:col-span-2 bg-white border border-slate-200/90 rounded-2xl p-6 shadow-2xs space-y-4">
    ...
    <!-- Ward Switcher & Break Glass Modals -->
    <WardSwitcherModal :isOpen="showWardSwitcher" @close="showWardSwitcher = false" @switched="loadOverview" />
...
import BreakGlassModal from '~/components/BreakGlassModal.vue'
