    ...
    <div class="pl-56 flex flex-col min-h-screen">
      ...
      <main class="flex-1 w-full px-8 py-6 space-y-5">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 class="font-brand text-xl font-semibold text-slate-900 tracking-tight">Clerk Profile & Credentials</h1>
            <p class="text-xs text-slate-500 mt-0.5">
              Review admissions desk shift parameters and manage personal account credentials.
            </p>
          </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="bg-white border border-slate-200/90 rounded-xl p-6 shadow-2xs space-y-6">
            <div class="flex items-center gap-4 pb-5 border-b border-slate-100">
              <div class="w-14 h-14 rounded-2xl bg-slate-900 text-white flex items-center justify-center font-bold text-xl font-mono shadow-inner">
                {{ initials }}
              </div>
              <div class="space-y-0.5 min-w-0">
                <h3 class="text-base font-bold text-slate-900 truncate">{{ user?.fullName || 'Clerk Staff' }}</h3>
                <p class="text-xs font-mono text-slate-500 truncate">@{{ user?.username }}</p>
                <div class="pt-1">
                  <RoleBadge :role="user?.role || 'CLERK'" />
                </div>
              </div>
            </div>
            ...
            <div class="space-y-3.5 text-xs">
              <div class="flex items-center justify-between py-1.5 border-b border-slate-100">
                <span class="text-slate-500">Home Department:</span>
                <span class="font-bold text-slate-800">{{ user?.homeWard?.name || 'Reception & Admissions' }}</span>
              </div>

              <div class="flex items-center justify-between py-1.5 border-b border-slate-100">
                <span class="text-slate-500">Active Working Ward:</span>
                <span class="font-bold text-blue-700 bg-blue-50 px-2 py-0.5 rounded font-mono">{{ activeWardCode }}</span>
              </div>
              ...
              <div class="flex items-center justify-between py-1.5 border-b border-slate-100">
                ...
                <span class="font-mono text-slate-700 font-bold">Standard Desk Shift</span>
              </div>

              <div class="flex items-center justify-between py-1.5">
                <span class="text-slate-500">Security Scope:</span>
                <span class="font-mono text-slate-700">Demographics & Bed Allocation</span>
              </div>
            </div>
          </div>
          ...
          <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white border border-slate-200/90 rounded-xl p-6 shadow-2xs space-y-5">
              <div class="flex items-center justify-between pb-3 border-b border-slate-100">
                <div>
                  <h3 class="text-xs font-bold text-slate-700 uppercase tracking-wider">Clerk Information</h3>
                  <p class="text-[11px] text-slate-500">Update your administrative display name on patient registrations.</p>
                </div>
              </div>
...
const activeWardCode = computed(() => activeWard.value?.code || 'MAIN')
...
const initials = computed(() => {
  const name = user.value?.fullName || user.value?.username || 'CL'
...
const handleUpdateProfile = async () => {
  ...
  try {
    ...
    successMsg.value = 'Full name updated successfully!'
  } catch (err: any) {
