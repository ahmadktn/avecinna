import pg from 'pg';
import { UserContext, CAACEvaluationResult } from '../types.js';

export class CAACEvaluator {
  ...
  /**
   * Evaluates CAAC authorization for a given user context and patient ID:
   ...
   */
  async evaluate(user: UserContext, patientId: string): Promise<CAACEvaluationResult> {
    const { userId, role, activeWardId, shiftStart, shiftEnd, isBreakGlass } = user;
    ...
    if (!this.pool) {
      throw new Error(
        'CAAC_DB_UNCONFIGURED: primaryDbUrl must be provided in SecurityMiddlewareOptions to evaluate CAAC rules against PostgreSQL.'
      );
    }
